...
with sudoku_tools_16;

Procedure sudoku_16 Is
    use JEWL.IO;
    ...
    -- Frame parameters
    FRAME_TITLE: String := "Sudoku_16x16";
    FRAME_WIDTH: Integer := 700;
    FRAME_HEIGTH: Integer := 765;

    -- Memo parameters
    MEMO_POSITION_X: Integer := 20;
    MEMO_POSITION_Y: Integer := 550;
    MEMO_WIDTH: Integer := 600;
    MEMO_HEIGHT: Integer := 150;

    -- Buttons parameters
    BUTTONS_POSITION_X: Integer := 550;
            ...
            if not canceled then
                if fillGrid(readFile(file), grid, base_filename) then
                    ...
                    if (Ada.Strings.Fixed.Index(Ada.Text_IO.Name(file), "\" & To_String(base_filename), 1) > 0) then
                        ...
                        Ada.Text_IO.Put_Line (Ada.Text_IO.Name(file) & " is user file");
    ...
    Procedure Init_Game (grid_blocks: in out Grid_Block_Type; memo: in Memo_Type) Is
        ...
        Begin
            Append_Line(memo, "Welcome to Sudoku 16x16 ! :)");
            createGrid(My_Frame, grid_blocks);
            base_filename := To_Unbounded_String("matrix_16.txt");
            if loadBaseMatrix(base_filename, grid) then
                base_filename := To_Unbounded_String("");
                setGrid(grid_blocks, grid, base_filename);
                Append_Line(memo, "Initial matrix_16.txt loaded.");
            end if;
    End Init_Game;
